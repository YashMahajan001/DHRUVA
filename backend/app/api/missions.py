...
@router.get("/active", response_model=MissionResponse)
def get_active_mission(db: Session = Depends(get_db)):
    """
    Get the current active (IN_PROGRESS) mission.
    Returns 404 if no mission is currently active.
    """
    from backend.app.models.mission import Mission as MissionModel

    active = (
        db.query(MissionModel)
        .filter(MissionModel.status == "IN_PROGRESS")
        .order_by(MissionModel.start_time.desc())
        .first()
    )
    if not active:
        # Fall back to most recent PLANNED mission if none IN_PROGRESS
        active = (
            db.query(MissionModel)
            .filter(MissionModel.status == "PLANNED")
            .order_by(MissionModel.start_time.desc())
            .first()
        )
    if not active:
